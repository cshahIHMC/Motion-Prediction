...
class Plotter:
    ...
    def plot_pca(self, ax, manifold_collection, title=None, point_color=None, draw_line=True, draw_arrow=True):
        ...
        num_steps = [len(m) for m in manifold_collection]
    ...
    def append_pca_gmm(self, ax, mean, variance, color=None, alphas=None):
        ...
        for i in range(n_components):
            ...
            ell = Ellipse(
                mu_transformed, width * std_scale, height * std_scale,
                angle=angle,
                fc=to_rgba(color, 0.2 * alphas[i]),
                ec=to_rgba(color, alphas[i]),
                lw=3,
            )
            ax.add_artist(ell)

    def plot_pca_intensity(self, ax, manifold_collection, values, cmap='YlOrRd',
                           vmin=0.0, vmax=1.0, xmin=None, xmax=None, ymin=None, ymax=None, title=None):
        ax.cla()
        num_steps = [len(m) for m in manifold_collection]
        manifolds = torch.cat(manifold_collection, dim=0).cpu()
        manifolds_pca = torch.tensor(self.pca.transform(manifolds)).split(num_steps, dim=0)
        for i, manifold in enumerate(manifolds_pca):
            ax.scatter(manifold[:, 0], manifold[:, 1], alpha=0.5, c=values[i].cpu(), cmap=cmap, vmin=vmin, vmax=vmax)
        ax.set_axis_off()
        if title is not None:
            ax.set_title(title)
    ...
    def plot_gmm(self, ax, data, pred_mean, pred_var, color=None, ymin=None, ymax=None, title=None):
        ...
        if pred_mean is not None and pred_var is not None:
            ...
            pred_var = (
                torch.linalg.cholesky(pred_var).diagonal(dim1=-2, dim2=-1)
                if pred_var.dim() == 3
                else pred_var
            )
